...
# （1）导数视频数据
wScr, hScr = autopy.screen.size()  # 返回电脑屏幕的宽和高(1706.0, 1066.0)
#print(wScr,hScr)

wCam, hCam = 1280, 720  # 视频显示窗口的宽和高
pt1, pt2 = (130, 130), (1120, 560)  # 虚拟鼠标的移动范围，左上坐标pt1，右下坐标pt2
pt3, pt4 = (110, 110), (1140, 580)
...
while True:
    ...
    cv2.rectangle(img, pt1, pt2, (67, 128, 6), 4)  #内框
    cv2.rectangle(img, pt3, pt4, (67, 128, 6), 6)  #外框
    ...
    if hands:
        ...
        # 只有食指和中指竖起，就认为是移动鼠标，距离来控制左键点击，右手指下压来表示右击鼠标
        if fingers[1] == 1 and fingers[2] == 1 and sum(fingers) == 2 and frame >= 1:
            ...
            if distance < 43 and frame >= 1:
                # 在食指尖画个绿色的圆，表示点击鼠标
                cv2.circle(img, (x1, y1), 16, (53, 194, 107), cv2.FILLED)

                # 左击鼠标
                autopy.mouse.click(button=autopy.mouse.Button.LEFT, delay=0)
                cv2.putText(img, "left_click", (500, 70), cv2.FONT_HERSHEY_PLAIN, 5, (67, 61, 89), 5)
                time.sleep(0.2)
                #print("左击鼠标")
            else:
                cv2.putText(img, "move", (500, 70), cv2.FONT_HERSHEY_PLAIN, 5, (67, 61, 89), 5)
        ...
        # 中指弯下食指在上，右击鼠标
            ...
            #print("右击鼠标")
            cv2.putText(img, "rigth_click", (500, 70), cv2.FONT_HERSHEY_PLAIN, 5, (67, 61, 89), 5)
            cv2.circle(img, (x2, y2), 16, (0, 255, 0), cv2.FILLED)
            time.sleep(0.2)
        ...
        # 五指紧握，按紧左键进行拖拽
            ...
            pLocx, pLocy = cLocx, cLocy
            cv2.putText(img, "drag", (500, 70), cv2.FONT_HERSHEY_PLAIN, 5, (67, 61, 89), 5)
            #print("拖拽鼠标")

        # 拇指张开，其他弯曲，按一次上键
        elif fingers == [1, 0, 0, 0, 0] and frame >= 2:
            cv2.putText(img, "UP", (500, 70), cv2.FONT_HERSHEY_PLAIN, 5, (67, 61, 89), 5)
        ...
        # 拇指弯曲，其他竖直，按一次下键
            ...
            cv2.putText(img, "Down", (500, 70), cv2.FONT_HERSHEY_PLAIN, 5, (67, 61, 89), 5)
            if (active_window_process_name == "cloudmusic.exe"):
                #print("#############################################")
        ...
        # 类ok手势，进行调整音量
            ...
            cv2.rectangle(img, (20, 150), (50, 350), (173, 202, 25), 2)
            cv2.rectangle(img, (20, int(volBar)), (50, 350), (173, 202, 25), cv2.FILLED)
            cv2.putText(img, f'{int(volPer * 100)}%', (10, 380), cv2.FONT_HERSHEY_PLAIN, 3, (173, 202, 25), 3)
    ...
    cv2.putText(img, f"FPS:{int(fps)}", (70, 50), cv2.FONT_HERSHEY_PLAIN, 3, (173,202,25), 3)
